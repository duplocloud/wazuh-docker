...
# This script prepares the Wazuh indexer package and configuration files.
# NOTE: The internal_users.yml file now uses placeholder values (e.g., "changeme")
# for password hashes. At container startup, the entrypoint.sh will generate bcrypt hashes
# from runtime-supplied plain passwords (via environment variables) and update this file accordingly.
...
## Variables to determine repository
...
## Check version to select the appropriate repository
...
# The internal_users.yml file (with placeholder hashes) will be updated at container runtime.
